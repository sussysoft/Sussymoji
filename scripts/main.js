...
leaveCookie(() => ({
	size: emoji.size.toString(),
	x: emoji.pos.x.toString(),
	y: emoji.pos.y.toString()
}));
...
function render() {
	...
	const width = emoji.img.width * emoji.size;
	const height = emoji.img.height * emoji.size;
	const left = (emoji.pos.x/100 * canvas.width) - width / 2;
	const top = canvas.height - ((emoji.pos.y/100 * canvas.height)) - (height / 2);

	
	// draw  face
	ctx.globalAlpha = 1;
	ctx.globalCompositeOperation = "source-over";
	ctx.drawImage(emoji.img, left, top, width, height);
...
function init() {
	...
	$("#horiz").val(emoji.pos.x)
		.on("input", function () {
			emoji.pos.x = Number($(this).val());
			render();
		});

	$("#vert").val(emoji.pos.y)
		.on("input", function () {
			emoji.pos.y = Number($(this).val());
			render();
		});
...
Promise.all([
	getImage("img/austo.png").then(img => emoji.img = img),
	...
	constructor() {
		...
		super({
			...
			ok: function() {
				...
				console.log("EMOJI = ", picker.value);
